...
### There is considerable skew as seen in the table
...
library(ROSE)
newdata <- ovun.sample(Bought ~ ., data = newdata, 
...
### Used Learning rate = 0.05, sample within training data is set at 75%
...
os_acc_2 <- sum(diag(table(preds2, actual2)))/sum(table(preds2, actual2))
